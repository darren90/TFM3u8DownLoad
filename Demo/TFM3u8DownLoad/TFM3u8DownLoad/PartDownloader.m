...
@implementation PartDownloader

static   PartDownloader *sharedFilesDownManage = nil;
...
-(void)beginRequest:(M3u8FileModel *)fileInfo isBeginDown:(BOOL)isBeginDown
{
...
#pragma mark - 启用ASIHTTPRequest进行下载请求
    ...
    BOOL exit = NO;
    for(ASIHTTPRequest *tempRequest in self.downinglist)  {
        M3u8FileModel *tempInfo =  [tempRequest.userInfo objectForKey:@"File"];
...
-(void)resumeRequest:(ASIHTTPRequest *)request{
    ...
    M3u8FileModel *fileInfo =  [request.userInfo objectForKey:@"File"];
    
    //中止一个进程使其进入等待
    for (M3u8FileModel *file in self.downinglist) {
...
#pragma mark- 保存片段到数据库
    float progress = 1 - [CommonHelper getProgress:self.partCont currentSize:self.m3u8PartList.count];
    ...
    for (M3u8FileModel *file in self.m3u8PartList) {
        if ([file.fileName isEqualToString:fileInfo.fileName]) {
...
-(void)deleteRequest:(ASIHTTPRequest *)request{
    ...
    NSFileManager *fileManager=[NSFileManager defaultManager];
    NSError *error;
    M3u8FileModel *fileInfo=(M3u8FileModel*)[request.userInfo objectForKey:@"File"];
...
#pragma mark - 下载进度的代理
    ...
    if (self.downinglist.count == 0) {//全部片段已经下载完毕
...
#pragma mark - 存储数据
-(void)saveDownloadFile:(M3u8FileModel *)fileinfo{
#pragma TODO - 下载 待做
...
//        [segments addObject:segment];
        ...
        M3u8FileModel *m = [[M3u8FileModel alloc]init];
        m.uniquenName = self.fileInfo.uniquenName;
        m.movieId = self.fileInfo.movieId;
        m.episode = self.fileInfo.episode;
        ...
        m.webPlayUrl = self.fileInfo.webPlayUrl;
        m.quality = self.fileInfo.quality;
        m.episodeSid = self.fileInfo.episodeSid;
        m.fileName = tsName;//self.fileInfo.fileName;//
        m.fileSize = self.fileInfo.fileSize;
        m.fileReceivedSize = self.fileInfo.fileReceivedSize;
        m.isFirstReceived = self.fileInfo.isFirstReceived;
        m.progress = self.fileInfo.progress;
        m.segmentHadDown = self.fileInfo.segmentHadDown;
        
//        //针对m3u8的格式(见本文最后)
//        if (![linkurl hasPrefix:@"http://"] || ![linkurl hasPrefix:@"https://"]) {//为类型2的格式，则进行url的拼接
//            //1:取出最后一个“/”后的XXXX.m3u8，
//            NSRange rang = [self.fileInfo.fileURL rangeOfString:@"/" options:NSBackwardsSearch];
//           linkurl = [self.fileInfo.fileURL stringByReplacingCharactersInRange:rang withString:linkurl];
//        }
...
-(NSString *)createLocalM3U8file
{
    ...
    [str enumerateStringsMatchedByRegex:httpPattern usingBlock:^(NSInteger captureCount, NSString *const __unsafe_unretained *capturedStrings, const NSRange *capturedRanges, volatile BOOL *const stop) {
        NSString* segmentPrefix = [NSString stringWithFormat:@"%@/%@/%@/",KLocaPlaylUrl,kDownMidPath,self.fileInfo.uniquenName];
    ...
    if(bSucc) {
        ...
        [[NSFileManager defaultManager] removeItemAtPath:tempTo error:nil];
    }
    else  {
...
#pragma mark - 下载完成的代理
    ...
    double downedSize = [WdCleanCaches biteSizeWithPaht:[NSString getHttpDowningSize:self.fileInfo.uniquenName urlType:self.fileInfo.urlType]];
